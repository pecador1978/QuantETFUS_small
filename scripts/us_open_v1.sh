...
SHARED="/Users/Finance/QuantShared/scripts"      # optional shared repo
LOGDIR="$ROOT/logs"
...
# -------- run --------
LOGFILE="$LOGDIR/us_open_v1_$(date -u +%Y%m%d_%H%M).log"
{
  say "=== US-OPEN REFRESH (v1) START ==="

  # Daily fetch/update (safe to run; no-op if nothing new)
  run_any "s10_ibkr_download_daily" \
  "$SCRIPTS/s10_ibkr_download_daily.py" "$SHARED/s10_ibkr_download_daily.py" \
  -- --client_id 87
  ...
  # Build/refresh daily contexts used by intraday enrichment
  ...
  # Latest intraday 30m + merge daily onto 30m
  run_any "s11_ibkr_download_30min" \
  "$SCRIPTS/s11_ibkr_download_30min.py" "$SHARED/s11_ibkr_download_30min.py" \
  -- --client_id 87
  run_py  "s32_enrich_30m_intraday"      "$SCRIPTS/s32_enrich_30m_intraday.py"

  # Integrity check (disabled for now – wrong path/name in your env)
  # run_any "00_data_integrity_enrichment" "$SCRIPTS/00_data_integrity_enrichment.py" "$SHARED/00_data_integrity_enrichment.py"
