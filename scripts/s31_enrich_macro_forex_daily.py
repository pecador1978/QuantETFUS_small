...
def main():
    ...
    ap.add_argument(
        "--forex_dir",
        type=str,
        default=str(P.FOREX_DIR),
        help="Directory with FX *_daily.csv (default: P.FOREX_DIR)",
    )
    ap.add_argument(
        "--macro_dir",
        type=str,
        default=str(P.MARKET_TRENDS_DIR),
        help="Directory with TV macro *_TV_daily.csv (default: P.MARKET_TRENDS_DIR)",
    )
    ap.add_argument(
        "--cfg",
        type=str,
        default=str(P.CONFIG_DIR / "ta_features.json"),
        help="Optional TA config JSON",
    )
    ap.add_argument(
        "--out_parquet",
        type=str,
        default=str(P.DATA_ENRICHED / "macro_forex_enriched.parquet"),
    )
    ap.add_argument(
        "--out_csv",
        type=str,
        default=str(P.DATA_ENRICHED / "macro_forex_enriched.csv"),
    )
    args = ap.parse_args()
    ...
    for f in sorted(files):
        try:
            ...
            # Close-based features (always safe)
            for n in cfg.get("EMA", []):
                g[f"ema{n}_d"] = ema(g["close"], n)
            for n in cfg.get("SMA", []):
                g[f"sma{n}_d"] = sma(g["close"], n)
            for n in cfg.get("RSI", []):
                g[f"rsi{n}_d"] = rsi(g["close"], n)

            # ATR / ADX / Stochastic — only if high/low present AND robust to schema quirks
            has_hl = {"high", "low"}.issubset(g.columns)
            if has_hl:
                try:
                    for n in cfg.get("ATR", []):
                        g[f"atr{n}_d"] = atr(g, n)
                    for n in cfg.get("ADX", []):
                        g[f"adx{n}_d"] = adx(g, n)
                    for st in cfg.get("Stochastic", []):
                        k = int(st.get("k", 14))
                        d = int(st.get("d", 3))
                        smooth = int(st.get("smooth", 3))
                        k_s, d_s = stochastic(g, k, d, smooth)
                        g[f"stoch_{k}_{d}_{smooth}_k_d"] = k_s
                        g[f"stoch_{k}_{d}_{smooth}_d_d"] = d_s
                except Exception as e:
                    # This is where uvol_dvol_ratio_TV_daily.csv used to blow up (KeyError 'high')
                    print(f"[WARN] TA (ATR/ADX/Stoch) failed for {f.name}: {e} — using close-only TA.")
            else:
                # No high/low -> skip ATR/ADX/Stoch entirely
                pass

            # Bollinger + MACD: close-only, safe for all instruments
            ...
            for m in cfg.get("MACD", []):
                fast   = int(m.get("fast", 12))
                slow   = int(m.get("slow", 26))
                signal = int(m.get("signal", 9))
