...
"""
s02_single_ticker_daily_data_download.py

!!! MARKET TRENDS TICKERS ONLY !!!

- Downloads ~10 years of DAILY bars for one ticker (default: XLE)
- TradingView CSV schema: time,open,high,low,close,Volume
- Output: /Users/Finance/QuantShared/market_trends/{TICKER}_TV_daily.csv
...
# where to write daily market-trends CSVs (global, not per-US/EU)
OUTPUT_DIR = Path(os.environ.get("MARKET_TRENDS_DIR", "/Users/Finance/QuantShared/market_trends"))
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

# Default mapping for XLE (adjust Currency to "GBP" if your line is GBP)
...
def fetch_daily_10y_with_fallback(ib: IB, contract: Contract, whats: List[str], rth: bool) -> pd.DataFrame:
    for what in whats:
        ...
        out = df[["datetime", "open", "high", "low", "close", "volume"]].dropna(subset=["datetime"]).sort_values("datetime")
        if not out.empty:
            return out
...
def main():
    ...
    out_path = OUTPUT_DIR / f"{ticker}_TV_daily.csv"
    ...
    try:
        ...
        df = fetch_daily_10y_with_fallback(ib, contr, whats=what_list(mp), rth=bool(args.use_rth))
        if df.empty:
            print(f"[WARN] {ticker}: no daily data (tried {what_list(mp)}).")
            return
        ...
        ib.disconnect()
        print("\n[DONE] Single-ticker daily download.")
