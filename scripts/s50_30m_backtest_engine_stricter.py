...
- Works for both EU and US clones.
- Entries: EMA/RSI/ATR/streak filters (no Vermeulen).
- Exits: T1 and T2 only, fixed 50/50 split. T2 may occur any day AFTER T1.
"""
...
OUT  = ROOT / "backtest_results_30m"   # mkdir done in main() to avoid import side-effects

# ---------------- helpers ----------------
def _filter_session_local(df_utc: pd.DataFrame) -> pd.DataFrame:
    if df_utc.empty: return df_utc
    df = df_utc.copy()
...
def _load_30min_any(ticker: str) -> pd.DataFrame:
    ...
    if not f.exists(): raise FileNotFoundError(f"30m file not found for {ticker} ({f_raw} / {f_alt})")
    df = pd.read_csv(f)
    df.columns = [c.strip().lower().replace(" ", "") for c in df.columns]
    dtcol = next((c for c in df.columns if "date" in c or "time" in c), None)
    if not dtcol: raise ValueError(f"No datetime column in {f}")
...
def _load_daily_any(ticker: str) -> pd.DataFrame:
    ...
    f = f_raw if f_raw.exists() else f_alt
    if not f.exists(): raise FileNotFoundError(f"Daily file not found for {ticker} ({f_raw} / {f_alt})")
    dfd = pd.read_csv(f)
    ...
    for c in ("open","high","low","close"):
        if c in dfd.columns: dfd[c] = pd.to_numeric(dfd[c], errors="coerce")
...
def apply_costs(px: float, side: str, slip_bps: float) -> float:
    if pd.isna(px) or px <= 0: return px
...
def _apply_window_ts(df: pd.DataFrame, ts_col: str, start: str | None, end: str | None) -> pd.DataFrame:
    if start: df = df[df[ts_col] >= pd.to_datetime(start, utc=True, errors="coerce")]
    if end:   df = df[df[ts_col] <= pd.to_datetime(end,   utc=True, errors="coerce")]
    return df

def _apply_window_date(df: pd.DataFrame, date_col: str, start: str | None, end: str | None) -> pd.DataFrame:
    if start: df = df[df[date_col] >= pd.to_datetime(start).date()]
    if end:   df = df[df[date_col] <= pd.to_datetime(end).date()]
    return df
...
def backtest_ticker(ticker: str, params: dict) -> pd.DataFrame:
    ...
    dfd["daily_ok"] = price_trend_ok & ema44_ok
    ...
    # relaxed streak: last bar close above EMA275 for prev 2 days
    ...
    dcols = ["date","ema5","ema20","ema44","momentum_breakout","daily_ok","ATR","atr_normalized","atr_surge"]
    ...
    momentum_ok = True if not bool(params.get("momentum_breakout_enabled", False)) else None
    df30["momentum_ok"] = True if momentum_ok is True else df30["momentum_breakout"].eq(True)
    ...
    for _, row in df30.iterrows():
        ...
        if current["T1_done"] and (not current["T2_done"]) and (row["datetime"].date() > current["T1_date"]) and high >= t2p:
            ...
            # raise trailing stop to T1 target after T2
            current["stop_level"] = t1p
            # if we fully exited with 50/50, position is done
...
def equity_curve(trades: pd.DataFrame, unit_capital: float) -> pd.DataFrame:
    if trades is None or trades.empty: return pd.DataFrame(columns=["date","equity"])
...
def main():
    ...
    print(f"[s50] TARGET_BUCKET={TARGET_BUCKET} | MARKET_TZ={MARKET_TZ} | MARKET_OPEN={MARKET_OPEN} | MARKET_CLOSE={MARKET_CLOSE}")
    ...
    (OUT / f"{prefix}_trades_{ts}.csv").parent.mkdir(parents=True, exist_ok=True)
