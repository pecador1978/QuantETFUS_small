...
(function(){
  const COL = {
    TICKER:0, AGE:1, TRADE:2, PHASE:3, TREND:4, STAGE:5,
    STR20:6, STR44:7, SENT:8, SM:9, RSI:10, ADX:11,
    P25_5:12, P75_5:13, CONF:14, BUCKET:15, REGIME:16,
    PUP1:17, PDN1:18, CIL1:19, CIH1:20, PUP3:21, PDN3:22, PUP5:23, PDN5:24,
    MED20:25, MED44:26, S50_20:27, S80_20:28, S50_44:29, S80_44:30, N:31
  };

  const numIdx = new Set([
    COL.AGE, COL.RSI, COL.ADX, COL.P25_5, COL.P75_5, COL.CONF,
    COL.PUP1, COL.PDN1, COL.CIL1, COL.CIH1, COL.PUP3, COL.PDN3, COL.PUP5, COL.PDN5,
    COL.MED20, COL.MED44, COL.S50_20, COL.S80_20, COL.S50_44, COL.S80_44, COL.N
  ]);

  const customOrder = new Map([[COL.BUCKET, ["HIGH","MED","LOW"]]]);

  function getCellText(td){ return (td && td.textContent ? td.textContent : '').trim(); }
  function parseNum(s){ const pct=s.replace(/[,%]/g,''); const num=pct.replace(/[^0-9.+-]/g,''); const v=parseFloat(num); return isNaN(v)?Number.NEGATIVE_INFINITY:v; }
  ...
  function sortTable(table, colIndex, asc){
    ...
    rows.sort((A,B)=>{
      ...
      let cmp; if(numIdx.has(colIndex)){ cmp = parseNum(a) - parseNum(b); } else { cmp = a.toLowerCase().localeCompare(b.toLowerCase()); }
  ...
  function filterRows(){
    ...
    for(const tr of rows){
      const hay = (tr.getAttribute('data-f') || '').toLowerCase();
  ...
  function init(){
    ...
    ths.forEach((th, idx)=>{
      ...
      th.addEventListener('click', ()=>{ const asc=!(th.dataset.asc==='true'); clearIndicators(ths); th.dataset.asc=String(asc); th.setAttribute('aria-sort', asc?'ascending':'descending'); th.querySelector('.sort-ind').textContent=asc?'▲':'▼'; sortTable(table, idx, asc); });
    });
    ths[COL.TICKER].dataset.asc='true'; ths[COL.TICKER].querySelector('.sort-ind').textContent='▲'; ths[COL.TICKER].setAttribute('aria-sort','ascending');
  }
  ...
  <div class="grid" style="display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:10px">
    <div class="box" style="background:#0f172a;border:1px solid #1f2937;border-radius:10px;padding:10px">
      ...
      <ul style="margin:0 0 0 16px;padding:0;font-size:12px;line-height:1.4">
        ...
        <li><b>Trend</b>: UP if EMA20 ≥ EMA44, else DOWN.</li>
        <li><b>AGE</b>: Consecutive <i>bars</i> since the trend signal started (day 2 of ≥EMA20 starts at 1).</li>
      </ul>
    </div>
    ...
    <div class="box" style="background:#0f172a;border:1px solid #1f2937;border-radius:10px;padding:10px">
      ...
      <ul style="margin:0 0 0 16px;padding:0;font-size:12px;line-height:1.4">
        <li><b>Strength-20/44</b>: % distance Close vs EMA20/EMA44 (from s77).</li>
        <li><b>RSI</b>: RSI(14).</li>
        <li><b>ADX</b>: ADX(14) trend strength; color threshold taken from s77 metadata (max of long/short mins).</li>
      </ul>
    </div>
    ...
    <div class="box" style="background:#0f172a;border:1px solid #1f2937;border-radius:10px;padding:10px">
      ...
      <ul style="margin:0 0 0 16px;padding:0;font-size:12px;line-height:1.4">
        ...
        <li><b>CI↓ / CI↑</b>: Calibrated 1-day <i>confidence interval bounds</i> for next-day move (lower/upper).</li>
    ...
    <div class="box" style="background:#0f172a;border:1px solid #1f2937;border-radius:10px;padding:10px">
      ...
      <ul style="margin:0 0 0 16px;padding:0;font-size:12px;line-height:1.4">
        <li><b>What</b>: Current “stretch” of price vs EMA20, classified using calibrated percentiles.</li>
        <li><b>How</b>: Compare today’s % distance (Close−EMA20)/EMA20 to the per-ticker historical distribution (p50 &amp; p80 thresholds).</li>
        <li><b>below_p50</b>: Within typical range (≤ median stretch). Lower mean-reversion risk.</li>
        <li><b>p50_to_p80</b>: Extended but not extreme (between median and 80th pct). Monitor / trim OK.</li>
        <li><b>above_p80</b>: Top 20% most stretched. Elevated pullback risk; avoid fresh longs / wait for pullback.</li>
        <li style="color:#94a3b8">Notes: Thresholds are per-ticker (calibrated). “p50@20 / p80@20” columns show the numeric cutoffs used.</li>
      </ul>
    </div>
    ...
    <div class="box" style="background:#0f172a;border:1px solid #1f2937;border-radius:10px;padding:10px">
      ...
      <ul style="margin:0 0 0 16px;padding:0;font-size:12px;line-height:1.4">
        <li><b>Emerging 🔥</b>: Trend just turned UP or price reclaimed EMA20 ≤3 bars ago; not stretched (S20≤2.5%, S44≤6%, RSI 45–65), not in “above_p80”.</li>
        ...
        <li><i>Near-20 🧪</i>: Optional watch—UP trend hovering near EMA20 (S20 −1%…+0.5%, RSI 40–55).</li>
      </ul>
    </div>
  </div>
</section>
