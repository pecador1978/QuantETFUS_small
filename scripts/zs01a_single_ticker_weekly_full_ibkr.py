...
"""
s01a_single_ticker_weekly_ibkr.py — FULL overwrite weekly download (IBKR schema)

!!! TRADABLE TICKERS ONLY !!!

- Pulls full window (default 10Y) of WEEKLY bars for one or many tickers.
- IBKR-format CSV: date,open,high,low,close,volume,average,barCount (UTC).
- Output: <project>/data_raw/<TARGET_BUCKET>/weekly/{TICKER}_weekly_raw.csv
"""
...
# --------------- config ---------------
HARDCODED_TICKERS: List[str] = ["AIGA"]  # edit if you want
DEFAULT_DURATION = "10 Y"
DEFAULT_USE_RTH = 1
DEFAULT_CCY = "USD" if PROJECT_ROOT.name.endswith("US_small") or PROJECT_ROOT.name.endswith("US") else "EUR"

# use shared raw store (US). For EU switch to data_raw_ETF_EU
SHARED_BASE = Path(os.environ.get("SHARED_RAW_BASE", "/Users/Finance/QuantShared/data_raw_ETF_US"))
OUT_DIR = SHARED_BASE / "weekly"
...
def fetch_full_weekly(ib: IB, contract: Contract, duration: str, what: str, use_rth: bool) -> pd.DataFrame:
...
def try_trades_midpoint(ib: IB, contract: Contract, duration: str, use_rth: bool, force_what: str | None) -> tuple[pd.DataFrame,str]:
    if force_what:
        df = fetch_full_weekly(ib, contract, duration, force_what.upper(), use_rth)
        return df, force_what.upper()
    for what in ("TRADES","MIDPOINT"):
        df = fetch_full_weekly(ib, contract, duration, what, use_rth)
        if not df.empty:
            return df, what
...
def main():
    ...
    for i, t in enumerate(tickers, 1):
        try:
            ...
            if df.empty:
                ...
                continue
            out_path = OUT_DIR / f"{t}_weekly_raw.csv"
    ...
    ib.disconnect()
    print("[DONE] s01a weekly full download complete.")
