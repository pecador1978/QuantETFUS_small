...
<script>
(function(){
  // Column typing: force TICKER as text; numeric for selected %/numbers columns.
  // Indexes: 0 TICKER, 1 TRADE, 2 PHASE, 3 TREND, 4 STR20, 5 STR44, 6 SENT, 7 SMART, 8 RSI, 9 ADX, 10/11 REASONS
  function getColType(idx){
    if (idx === 0) return 'text';           // TICKER => always text
    if (idx === 4 || idx === 5 || idx === 8 || idx === 9) return 'num'; // numeric-ish
    return 'text';                           // everything else as text
  }
  ...
  function parseCell(txt, forcedType){
    ...
    // Fallback auto (unused now, but kept)
    const pct = s.replace(/[,%]/g,'');
  ...
  function init(){
    ...
    // Default: sort by BUY/SELL grouping server-side; then show UI as 'Ticker ▲'
    const tickerIdx = 0;
